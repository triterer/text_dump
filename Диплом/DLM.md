...
Для синхронизации изменений между кластерами будем применять следующий подход. Есть мастер база, остальные слейв реплики. На slave репликах выполняются чтения и для ни локально проставляются блокировки в местных инстансах DLM. Но для выполнения эксклюзивных блокировок эти транзакии отсылаются на мастер кластер и там резолвятся как обычно. Любые эксклюзивные изменения приходят на все слейв кластера как приоритетные записи. Они выполняются в первую очередь, когда все замки этих транзакций успешно проставлены. Все локальные чтения ждут завершения таких изменений.

Про эскалацию блокировок:
Когда мы проводим сканирование и проставляем SIX блокировки, то мы потенциально можем проставлть SIX на общую схему. Но я вроде отказался от общей схемы.